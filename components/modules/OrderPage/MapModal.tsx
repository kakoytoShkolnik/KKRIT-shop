...
import { handleResultClearing, handleResultSelection, handleResultsFound, handleSelectPickupAddress, SearchMarkersManager } from "@/lib/utils/map"
import { mapOptions } from '@/constants/map'
import { motion } from "framer-motion"
import { basePropsForMotion } from "@/constants/motion"
import AddressesList from "./AddressesList"
import { $chosenPickupAddressData, $courierAddressData, $mapInstance, $pickUpPointDataByCity, $shouldShowCourierAddressData } from "@/context/order/state"
import { useTTMap } from "@/hooks/useTTMap"
import { setChosenCourierAddressData, setCourierAddressData, setShouldLoadPickUpPointData, setShouldShowCourierAddressData } from "@/context/order"
import { $userGeolocation } from "@/context/users/state"
import { IPickUpPointAddressData } from "@/types/order"
import CourierAddressesItem from "./CourierAddressesItem"
import { getGeolocationFx } from "@/context/users"
...
const MapModal = () => {
    ...
    const handleLoadMap = async (initialContainer = pickUpMapRef) => {
        ...
        const setMarkersByLocationsData = (data: IPickUpPointAddressData[]) => {
            data.forEach((item) => {
        ...
        if (!!chosenPickupAddressData.address_line1) {
            const chosenItem = pickUpPointDataByCity.filter(
        ...
        if (pickUpPointDataByCity.length) {
            setMarkersByLocationsData(pickUpPointDataByCity)
        }
